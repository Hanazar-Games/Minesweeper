...
### 改进
- **主菜单布局优化**：移除 `grid-row: span 2` 导致的 masonry 错乱效果，全部按钮统一为正方形卡片
  - 6 列网格占满整个页面宽度（`max-width: 1100px`），左右留白均匀
  - 主要模式通过渐变色背景 + 主题色边框 + 更大图标突出，不再依赖尺寸差异
  - 响应式调整：桌面 6 列 → 平板 4 列 → 手机 3 列

### Bug 修复
- **parseInt 缺少 radix 参数**：`game.js` 和 `ui.js` 中多处 `parseInt(value)` 未指定进制，用户输入 `"08"` 时旧浏览器可能按八进制解析得到 `0`。已全部补全为 `parseInt(value, 10)`
...
### 新功能
- **品牌入场动画**：游戏启动时新增 `Hanazar Products` → `minesweeper / 超级扫雷` 品牌入场动画
  - 深黑背景 + 渐进式淡入动画：品牌名 → 分隔线展开 → 游戏名
  - 入场动画持续约 2.5 秒，随后平滑过渡到原有启动画面
  - 同步支持中/英/日三种语言（`intro_brand`、`intro_game_name_en`、`intro_game_name`）
  - 紧急后备机制同步覆盖入场动画，确保 JS 加载失败时不会卡住
